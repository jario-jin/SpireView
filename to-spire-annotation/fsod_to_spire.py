...
def main():
    ...
    parser.add_argument(
        "--coco-anno",
        default="/home/jario/dataset/coco/Few-Shot-Object-Detection-Dataset/fsod/test.json",
        help="path to coco annotation file",
        # required=True
    )
    parser.add_argument(
        "--coco-image-dir",
        default="/home/jario/dataset/coco/Few-Shot-Object-Detection-Dataset/fsod/all_images",
        help="path to coco image dir",
    )
    parser.add_argument(
        "--output-dir",
        default="/tmp/fsod_spire",
    ...
    for img_info in tqdm(json_dict['images']):
        ...
        # notice this
        origin_img_fn = img_name  # os.path.join(args.coco_image_dir, img_name)
        ...
        for anno in img_annos:
            ...
            if 'segmentation' in spire_anno.keys() and isinstance(spire_anno['segmentation'], list) and \
                    len(spire_anno['segmentation']) > 0:
                spire_dict['annos'].append(spire_anno)
        ...
        if args.save_image:
            open(os.path.join(scaled_images, img_name), 'wb').write(
                open(origin_img_fn, 'rb').read())

        # Show image for testing.
        if args.show_image:
            img = cv2.imread(origin_img_fn)
            cv2.imshow('img', img)
            cv2.waitKey(100)
            print(img_name)
