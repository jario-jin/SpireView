...
def main():
    parser = argparse.ArgumentParser(description="Convert Pascal VOC annotation to spire annotation")
    parser.add_argument(
        "--pascalvoc-main-txt",
        default="D:\\uav-dataset\\uav-dataset\\comic\\ImageSets\\Main\\test.txt",
        help="path to Pascal VOC annotation file",
    )
    parser.add_argument(
        "--pascalvoc-anno",
        default="D:\\uav-dataset\\uav-dataset\\comic\\Annotations",
        help="path to Pascal VOC annotation file",
    )
    parser.add_argument(
        "--pascalvoc-image-dir",
        default="D:\\uav-dataset\\uav-dataset\\comic\\JPEGImages",
        help="path to Pascal VOC image dir",
    )
    parser.add_argument(
        "--output-dir",
        default="D:\\Dataset\\BB201205-Comic-val",
    ...
    # xml_names = os.listdir(args.pascalvoc_anno)
    f = open(args.pascalvoc_main_txt, 'r')
    anno_lines = f.readlines()

    # for xml_name in tqdm(xml_names):
    for xml_name in tqdm(anno_lines):
        xml_name = xml_name.strip() + '.xml'
        if xml_name[-4:] == '.xml':
            image_fn = os.path.join(args.pascalvoc_image_dir, xml_name[:-4] + '.jpg')
            image = cv2.imread(image_fn)
            xml_fn = os.path.join(args.pascalvoc_anno, xml_name)
            # 读取XML文件
            ...
            image_fn = os.path.join(args.pascalvoc_image_dir, filename)
